        ...
        while len(l) > 0: #enquanto a lista l não for vazia
            ...
            for elem in self.graph[node]: #corre todos os tuplos do node
                ...
                if elem[1] < bestcr: #sempre que o score for mais baixo do que o anterior
                    ...
                    novo_node = elem[0] #atualizar o nó destino
            if novo_node not in visited: #se o novo node ainda não tiver sido visitado
                l.append((novo_node, preds + [node], score + bestcr)) #adiciona-se esse node, o caminho até ao momento e o score acumulado à lista l 
...
## cycles
    def node_has_cycle (self, v): #verifica se o nó está num ciclo, ou seja, se aparece mais do que uma vez
        l = [v]
        res = False
        visited = [v]
        while len(l) > 0:
            node = l.pop(0)
            for elem in self.graph[node]:
                if elem[0] == v: #se o vertice dado voltar a ocorrer no grafo, significa que há um ciclo
                    return True
    ...
    def has_cycle(self): #verificar se o graph é ciclico ou não
        res = False
        for v in self.graph.keys(): #corre todos os nodesdo grafo
            if self.node_has_cycle(v): #verifica se o node do ciclo está num ciclo ou não
                return True
        return res


def is_in_tuple_list (tl, val): #verificar se um valor está dentro da lista de tuplos
    res = False
    for (x,y) in tl:
