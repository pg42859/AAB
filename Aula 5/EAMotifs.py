...
class EAMotifsInt (EvolAlgorithm):
    ...
    def __init__(self, popsize, numits, noffspring, filename):
        ...
        indsize = len(self.motifs) #numero de seqs a analisar aka numero de individuos
        EvolAlgorithm.__init__(self, popsize, numits, noffspring, indsize)

    def initPopul(self, indsize):
        maxvalue = self.motifs.seqSize(0) - self.motifs.motifSize #upper bound é o indice maximo que o vetor das pos pode ter (tamanho da seq - tamanho do motif)
...
class EAMotifsReal (EvolAlgorithm): #tem todas as funções da class anterior

    def __init__(self, popsize, numits, noffspring, filename):
        self.mottifes = MotifFinding() #iniciar o MotifFiding
        self.motifs.readFile(filename, "dna") #guarda as seqs
        indsize = self.motifs.motifSize * len(self.motifs.alphabet) #tamanho vai ser um vetor da pwm, linhas( tamanho alfabeto) e colunas(tamanho motif)
        EvolAlgorithm.__init__(self, popsize, numits, noffspring, indsize)

    def initPopul(self, indsize): #override da mesma função da classe anterior
        low = 0
        up = self.motifs.seqSize(0) - self.motifs.motifSize #indice maximo do vetor de pos
        self.popul = PopulReal(self.popsize, indsize, low, up, [])

    def v_pwm(self, v): #transformar em pwm a partir de um vetor de um indiv
        tam_mot = self.motifs.motifSize #tamanho motif
        tam_alf = len(self.motifes.alphabet) #tamanho alfabeto
        pwm = createMatZeros(tam_alf, tam_mot) #criar pwm de zeros tendo em conta os parametros acima provenientes da classe MotifFinding
        for a in range(0, len(v), tam_alf): #correr o vetor em incrementos de tamanho do alfabeto
                id_col = int(a / tam_alf) #indice vai incrementar 1 a 1, int porque decimal não pode entrar na matriz
    ...
    def mostProbableSeq(self, seq):
        ...
        for a in range(len(seq) - self.motifs.motifSize): #corre enquanto o motif cabe na seq
            prob = self.probabSeq(seq[a : a + self.motifs.motifSize]) #probabilidade de cada subseq
